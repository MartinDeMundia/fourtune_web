    ...
    <div class="md-layout-item md-size-100">
      <md-card>
        <md-card-header class="md-card-header-icon md-card-header-green">
          ...
          <h4 class="title">Fourtune Events Listings</h4>
        </md-card-header>
        <md-card-content>
          ...
          >
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              <md-table-cell md-label="Event Date" md-sort-by="event_date">{{item.event_date}}</md-table-cell>
              <md-table-cell md-label="Title" md-sort-by="event_name">{{item.event_name}}</md-table-cell>
              <md-table-cell md-label="Venue" md-sort-by="event_location">{{item.event_location}}</md-table-cell>
              <md-table-cell md-label="Fourtune Token" md-sort-by="price">{{item.price}}</md-table-cell>
...
import * as $ from "jquery";
  ...
  export default {
    ...
    data: () => ({
      table: [],
      footerTable: ["Event Date","Title","Venue","Fourtune Token"],
      query: null,
      sortation: {
        field: "created_at",
        order: "asc"
    ...
    onDeleteItem(selectedObject){
        var params = {     
          ...
          return axios.post(`${url}/delete-fourtune-event`, params, options)
          .then(response => { 
                  this.$store.dispatch("alerts/success", "Successfully deleted record!");
                  this.table = response.data.events_data;                 
         }); 
      },
      getList() {
          var params = {"id":1};
