    ...
    <div class="md-layout-item md-size-100">
      <md-card>
        <md-card-header class="md-card-header-icon md-card-header-green">
          ...
          <h4 class="title">View How Gamers Transacted/Transferd Fourtune Crypto</h4>
        </md-card-header>
        <md-card-content>
          ...
          >
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              <md-table-cell md-label="From Name" md-sort-by="fromname">{{item.fromname}}</md-table-cell>
              <md-table-cell md-label="Phone" md-sort-by="fromphone">{{item.fromphone}}</md-table-cell>
              <md-table-cell md-label="To Name" md-sort-by="toname">{{item.toname}}</md-table-cell>
              <md-table-cell md-label="Phone" md-sort-by="tophone">{{item.tophone}}</md-table-cell>
              <md-table-cell md-label="Amount Transfered" md-sort-by="amount">{{item.amount}}</md-table-cell>
              <md-table-cell md-label="Source Totals" md-sort-by="totals_from">{{item.totals_from}}</md-table-cell>
              <md-table-cell md-label="Destination Totals" md-sort-by="totals_to">{{item.totals_to}}</md-table-cell> 
            </md-table-row>
          </md-table>
...
import * as $ from "jquery";
  ...
  export default {
    ...
    data: () => ({
      table: [],
      footerTable: ["From Name","Phone","To Name","Phone","Amount Transfered","Source Totals", "Destination Totals"],
      query: null,
      sortation: {
        field: "created_at",
        order: "asc"
    ...
    methods: {      
      getList() {
          ...
          return axios.post(`${url}/get-fourtune-transfer`, params, options)
          .then(response => { 
                  this.table = response.data.tokentransfer;
         });    
